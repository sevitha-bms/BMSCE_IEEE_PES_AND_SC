...
const validateTime = (timeString) => {
  ...
  if (!isValid) {
    window.alert("Incorrect Time Format");
    return false;
  }
  return true;
};
...
export default function PESDayInput({
  ...
  const handleSubmit = async (submitEvent) => {
    ...
    try {
      let response;
      if (event._id) {
        response = await axios.post("/api/PES_DAY/update", data);
      } else {
        response = await axios.post("/api/PES_DAY/input", data);
      }
